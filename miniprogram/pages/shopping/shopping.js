...
Page({
  ...
  // 存在问题：价格只能是数字，无法携带产品其他参数如100元/g等
  //     }
  //   }
  //   that.setData({
  //     Money:Money
  ...
  countMoney(){
    ...
    var array = that.data.array;
    var Money = 0;
    var pricePattern = /(\d+(\.\d+)?)/; // 匹配价格中的数字部分
    for(var i = 0; i < array.length; i++){
      if(array[i].checked == true){
        var priceStr = array[i].price; // 获取价格字符串
        var matchResult = priceStr.match(pricePattern); // 匹配数字部分
        var price = matchResult ? parseFloat(matchResult[0]) : 0; // 将数字部分转换为浮点数
        Money += price * array[i].num; // 计算商品价格
      }
    }
    that.setData({
      Money:Money
